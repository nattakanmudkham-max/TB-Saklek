...
export default function EditContactPage() {
    ...
    async function load() {
      ...
      if (data) {
        ...
        setForm({
          ...
          sputum_afb: s(data.sputum_afb), genexpert: s(data.genexpert),
          igra: s(data.igra), notes: s(data.notes),
        })
      }
      setLoading(false)
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('contacts').update(payload).eq('id', params.id)
    setSaving(false)
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 36, height: 36, background: '#fef9c3', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>👥</div>
              <div>
                <h1 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', margin: 0 }}>แก้ไขข้อมูลผู้สัมผัส</h1>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0', padding: '28px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <div className="grid grid-cols-2 gap-5">
              ...
              <FormInput label="ลำดับที่" type="number" value={form.contact_seq} onChange={e => set('contact_seq', e.target.value)} />
