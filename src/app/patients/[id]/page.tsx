...
export default function EditPatientPage() {
  ...
  const [form, setForm] = useState({
    ...
    icd10: '', xpert_result: '', is_ip: 'false', is_ep: 'false',
    ...
    async function load() {
      ...
      if (data) {
        setForm({
          ...
          title: data.title ?? '',
          // put full_name in first_name if title/first_name not stored separately
    ...
    <div style={{ minHeight: '100vh', background: '#f1f5f9' }}>
      <div style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '18px 32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={handleDelete} disabled={deleting} style={{ background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', padding: '7px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
            🗑 ลบข้อมูล
          </button>
        </div>
      </div>
      ...
      <div style={{ padding: '28px 32px', maxWidth: 980 }}>
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0', marginBottom: 16, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ padding: '20px' }}>
              <div className="grid grid-cols-4 gap-4">
                <FormInput label="การวินิจฉัย (ICD-10)" value={form.icd10} onChange={e => set('icd10', e.target.value)} placeholder="เช่น A150" />
