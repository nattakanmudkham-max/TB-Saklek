...
export default function PatientsPage() {
  ...
  const filtered = patients.filter(p => {
    ...
    const matchIP =
      filterIP === '' ? true :
      filterIP === 'ในปอด' ? p.is_ip :
      filterIP === 'นอกปอด' ? p.is_ep : true
      ...
      <div style={{ padding: '24px 32px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ position: 'relative', flex: 1, maxWidth: 320 }}>
            ...
            <input
              ...
              style={{ width: '100%', border: '1px solid #e2e8f0', borderRadius: 10, padding: '9px 12px 9px 34px', fontSize: 13, background: '#fff', outline: 'none', color: '#334155' }}
            />
          </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
          <select value={filterIP} onChange={e => setFilterIP(e.target.value)} style={selectStyle}>
            <option value="">ในปอด/นอกปอด (ทั้งหมด)</option>
            <option value="ในปอด">ในปอด</option>
            <option value="นอกปอด">นอกปอด</option>
          </select>
          ...
          <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                {['ลำดับ', 'TB No.', 'HN', 'วันที่ลงทะเบียน', 'ชื่อ-สกุล', 'อายุ', 'การวินิจฉัย (ICD-10)', 'ในปอด/นอกปอด', 'วันเริ่มรักษา', 'ประเภท', 'ผลการรักษา', ''].map(h => (
                  <th key={h} style={{
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={colSpan} style={{ textAlign: 'center', padding: '48px', color: '#94a3b8', fontSize: 13 }}>
                  <div>⏳ กำลังโหลด...</div>
                </td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={colSpan} style={{ textAlign: 'center', padding: '48px', color: '#94a3b8', fontSize: 13 }}>
                ...
                return (
                  <tr key={p.id} style={{
                    ...
                    <td style={{ padding: '11px 14px', fontFamily: 'monospace', fontSize: 11, color: '#475569', fontWeight: 600, whiteSpace: 'nowrap' }}>
                      {p.tb_no ? String(Math.round(parseFloat(String(p.tb_no)))) : '-'}
                    </td>
                    <td style={{ padding: '11px 14px', fontFamily: 'monospace', fontSize: 12, color: '#475569', fontWeight: 600, whiteSpace: 'nowrap' }}>
                      {p.hn ? String(Math.round(parseFloat(String(p.hn)))).padStart(9, '0') : '-'}
                    </td>
                    <td style={{ padding: '11px 14px', color: '#475569', whiteSpace: 'nowrap', fontSize: 12 }}>
                      {toThaiBE(p.registered_date)}
                    </td>
                    <td style={{ padding: '11px 14px', fontWeight: 600, color: '#0f172a' }}>{p.full_name}</td>
                    <td style={{ padding: '11px 14px', color: '#475569', textAlign: 'center' }}>{p.age ?? '-'}</td>
                    <td style={{ padding: '11px 14px', color: '#475569' }}>{p.icd10 || '-'}</td>
                    <td style={{ padding: '11px 14px' }}>
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                        {p.is_ip && <span style={{ background: '#dbeafe', color: '#1d4ed8', padding: '2px 7px', borderRadius: 5, fontSize: 11, fontWeight: 600, whiteSpace: 'nowrap' }}>ในปอด</span>}
                        {p.is_ep && <span style={{ background: '#ffedd5', color: '#c2410c', padding: '2px 7px', borderRadius: 5, fontSize: 11, fontWeight: 600, whiteSpace: 'nowrap' }}>นอกปอด</span>}
                        {!p.is_ip && !p.is_ep && <span style={{ color: '#cbd5e1', fontSize: 11 }}>-</span>}
                      </div>
                    </td>
                    <td style={{ padding: '11px 14px', color: '#475569', whiteSpace: 'nowrap', fontSize: 12 }}>
                      {toThaiBE(p.treatment_start_date)}
                    </td>
                    <td style={{ padding: '11px 14px' }}>
                      <span style={{ background: '#dbeafe', color: '#1d4ed8', padding: '2px 8px', borderRadius: 6, fontSize: 11, fontWeight: 600, whiteSpace: 'nowrap' }}>
                        {p.patient_type || '-'}
                      </span>
                    </td>
                    <td style={{ padding: '11px 14px' }}>
                      <span style={{ background: oc.bg, color: oc.text, padding: '2px 8px', borderRadius: 6, fontSize: 11, fontWeight: 600, whiteSpace: 'nowrap' }}>
                        {p.treatment_outcome || '-'}
                      </span>
                    </td>
                    <td style={{ padding: '11px 14px' }}>
